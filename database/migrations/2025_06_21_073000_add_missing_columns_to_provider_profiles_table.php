...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('provider_profiles', function (Blueprint $table) {
            // First, drop the existing primary key and foreign key
            $table->dropPrimary(['provider_id']);
            $table->dropForeign(['provider_id']);

            // Add UUID id column as new primary key
            $table->uuid('id')->primary()->first();

            // Make provider_id a regular foreign key column (not primary)
            $table->unsignedBigInteger('provider_id')->nullable(false)->change();
            $table->foreign('provider_id')->references('id')->on('users')->onDelete('cascade');
            $table->unique('provider_id'); // Ensure one profile per provider

            // Add azure_provider_fhir_id column if it doesn't exist
            if (!Schema::hasColumn('provider_profiles', 'azure_provider_fhir_id')) {
                $table->string('azure_provider_fhir_id')->nullable()->after('provider_id');
            }
    ...
    public function down(): void
    {
        Schema::table('provider_profiles', function (Blueprint $table) {
            ...
            // Restore provider_id as primary key
            $table->unsignedBigInteger('provider_id')->primary()->change();
            $table->foreign('provider_id')->references('id')->on('users')->onDelete('cascade');
        });
    }
};
