 ...
 * - NO business rules
 */
class QuickRequestOrchestratorV2
{
    ...
    protected function extractEpisodeData(PatientManufacturerIVREpisode $episode, array $additionalData): array
    {
        ...
        // Use EntityDataService to extract additional data based on IDs
        if (!empty($rawData['provider_id']) || !empty($rawData['facility_id'])) {
            $entityData = $this->entityDataService->extractDataByRole(
                Auth::id(),
                $rawData['facility_id'] ?? null,
                $rawData['provider_id'] ?? null,
                [] // Let the service determine what fields to extract
            );
            
            $rawData = array_merge($rawData, $entityData);
        }
